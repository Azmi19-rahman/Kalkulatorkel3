...
# ==============================================================================
# 🗃️ INISIALISASI DATABASE FISIK (SQLITE MULTI-USER)
# ==============================================================================
def init_db():
...
# --- FUNGSI QUERY SECURITY & USER ---
def register_user(username, password):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO users VALUES (?, ?)", (username, make_hashes(password)))
        conn.commit()
        sukses = True
...
# --- FUNGSI QUERY LOG DATA SPESIFIK USER ---
def save_water_log(username, id_biner, sampel, parameter, nilai, status, keterangan):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO water_log VALUES (?, ?, ?, ?, ?, ?, ?)", (username, id_biner, sampel, parameter, nilai, status, keterangan))
    conn.commit()
    conn.close()
...
def get_water_logs(username):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT id_biner, sampel, parameter, nilai, status, keterangan FROM water_log WHERE username = ?", (username,))
    rows = cursor.fetchall()
    conn.close()
    return [{"id_biner": r[0], "sampel": r[1], "parameter": r[2], "nilai": r[3], "status": r[4], "keterangan": r[5]} for r in rows]

def clear_water_logs(username):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM water_log WHERE username = ?", (username,))
    conn.commit()
    conn.close()
...
# ==============================================================================
# 🧠 LOGIKA EVALUASI AI (FORMAT PARAGRAF KONTINU SPESIFIK USER)
# ==============================================================================
def ai_water_evaluation(username, data_baru, batas_acuan, parameter_nama, tipe_ambang="maks"):
    ...
    if len(data_sejenis) >= 3:
        ...
        pembahasan += "Saat ini AI belum mengaktifkan modul analitik prediktif mendalam dikarenakan jumlah data sampel valid yang tersimpan di akun anda masih kurang dari tiga rekaman historis."
        
    return pembahasan

def ai_chatbot_brain(username, pertanyaan):
    pertanyaan = pertanyaan.lower().strip()
    memori_pengetahuan = get_ai_knowledge()
    database_air = get_water_logs(username)
    
    if pertanyaan in ["halo", "hai", "p", "test", "halo ai"]:
        return f"Sini masuk, kak {username}! Ada data lab apa yang mau kita beresin bareng hari ini? 💧"
    ...
    if "rekap" in pertanyaan or "evaluasi" in pertanyaan or "total" in pertanyaan:
        if not database_air: 
            return "Waduh, database riwayat di akunmu masih kosong nih. Yuk, coba hitung dan simpan satu sampel dulu!"
        
        total = len(database_air)
        reject = sum(1 for d in database_air if d["status"] in ["MELEBIHI AMBANG", "DI BAWAH MINIMUM"])
        
        respons = f"**[Database Report - Akun {username}]:** Oke, mari kita cek isi harddisk! Total riwayat pengujianmu ada **{total} sampel**. "
        if reject > 0:
...
st.markdown(f"""
    <style>
    .stApp {{ {bg_style} color: #f8fafc !important; }}
    ...
    label, p, span, div, .stMarkdown {{ color: #e2e8f0 !important; }}
    h1, h2, h3, h4, h5, h6 {{ color: #f8fafc !important; }}
    .stButton>button {{
        background: linear-gradient(45deg, #0284c7, #3b82f6) !important; color: white !important;
        border-radius: 8px !important; border: 1px solid rgba(56, 189, 248, 0.4) !important; font-weight: 600 !important;
        box-shadow: 0 4px 15px rgba(2, 132, 199, 0.3) !important;
    }}
    .stTable table {{ background-color: rgba(30, 41, 59, 0.4) !important; color: #f1f5f9 !important; }}
    </style>
""", unsafe_allow_html=True)


# ==============================================================================
# 🔐 LOGIKA LOGIN & LOGOUT FRONTEND
# ==============================================================================
...
if not st.session_state["logged_in"]:
    ...
    with col_log2:
        tab_login, tab_regis = st.tabs(["🔑 Masuk Akun", "📝 Daftar Akun Baru"])
        ...
        with tab_login:
            ...
            if st.button("Masuk Dashboard", use_container_width=True):
                if login_user(user_in, pass_in):
                    ...
                    st.toast(f"Selamat datang kembali, {user_in}! 👋")
                    st.rerun()
                else:
            ...
            if st.button("Daftarkan Akun", use_container_width=True):
                if reg_user and reg_pass:
                    if register_user(reg_user, reg_pass):
                        ...
                        st.error("Username sudah terpakai. Gunakan nama unik lain!")
                else:
...
# --- TAMPILAN JIKA SUDAH LOGIN ---
    ...
    # --- SIDEBAR (NAVIGASI) ---
    with st.sidebar:
        st.markdown(f"<h2 style='color: #38bdf8; margin-bottom: 0px; font-weight:800;'>💧 Akun: {current_user}</h2>", unsafe_allow_html=True)
        ...
        logs_saat_ini = get_water_logs(current_user)
        ...
        st.metric("Total Sampel Kamu", f"{total_data} Sampel")
        ...
        st.markdown("---")
        if st.button("🚪 Keluar Sesi Akun", use_container_width=True):
            st.session_state["logged_in"] = False
            st.session_state["username"] = ""
            st.rerun()
    ...
    # 🧪 PERHITUNGAN BOD
        ...
        with col_l1:
            ...
            if st.button("🔥 Hitung & Simpan Data BOD", use_container_width=True):
                ...
                save_water_log(current_user, biner_id, nama_smpl, "BOD", hasil, status, f"DO0={do_0}, DO5={do_5}, P={f_pengenceran}")
                st.session_state["pembahasan_bod"] = ai_water_evaluation(current_user, {"id_biner": biner_id, "parameter": "BOD", "nilai": hasil, "status": status}, bod_max, "BOD", "maks")
    ...
    # 🧪 PERHITUNGAN COD
        ...
        with col_l1:
            ...
            if st.button("🔥 Hitung & Simpan Data COD", use_container_width=True):
                ...
                save_water_log(current_user, biner_id, nama_smpl, "COD", hasil, status, f"V_B={v_blanko}, V_S={v_sampel}, N={n_fas}")
                st.session_state["pembahasan_cod"] = ai_water_evaluation(current_user, {"id_biner": biner_id, "parameter": "COD", "nilai": hasil, "status": status}, cod_max, "COD", "maks")
    ...
    # ⚖️ PERHITUNGAN TSS
        ...
        with col_n1:
            ...
            if st.button("🔥 Hitung & Simpan Data TSS", use_container_width=True):
                ...
                save_water_log(current_user, biner_id, nama_smpl_baru, "TSS", hasil, status, f"B_Awal={b_awal} g, B_Akhir={b_akhir} g")
                st.session_state["pembahasan_tss"] = ai_water_evaluation(current_user, {"id_biner": biner_id, "parameter": "TSS", "nilai": hasil, "status": status}, tss_max, "TSS", "maks")
    ...
    # 🧪 PERHITUNGAN DO
        ...
        with col_n1:
            ...
            if st.button("🔥 Hitung & Simpan Data DO", use_container_width=True):
                ...
                save_water_log(current_user, biner_id, nama_smpl_baru, "DO", hasil, status, f"V_Thio={v_thio} mL, N={n_thio}")
                st.session_state["pembahasan_do"] = ai_water_evaluation(current_user, {"id_biner": biner_id, "parameter": "DO", "nilai": hasil, "status": status}, do_min, "Dissolved Oxygen (DO)", "min")
        ...
        with col_a1:
            ...
            if chat_in:
                with st.chat_message("assistant"):
                    st.write(ai_chatbot_brain(current_user, chat_in))
        with col_a2:
